...
import uos
import urequests
from config import HOST, DEVICE_ID

# Base project url
# https://github.com/olivergregorius/micropython_ota
__author__ = "Oliver Gregorius <oliver@gregorius.dev>"


def check_version(auth=None, timeout=5) -> (bool, str):
    current_version = ''
    try:
        ...
        if auth:
            response = urequests.get(f'{HOST}/{DEVICE_ID}/version', headers={'Authorization': f'Basic {auth}'}, timeout=timeout)
        else:
            response = urequests.get(f'{HOST}/{DEVICE_ID}/version', timeout=timeout)
        response_status_code = response.status_code
        response_text = response.text
        response.close()
        if response_status_code != 200:
            print(f'Remote version file {HOST}/{DEVICE_ID}/version not found')
...
def ota_update(filenames, use_version_prefix=True, user=None, passwd=None, hard_reset_device=True, soft_reset_device=False, timeout=5) -> None:
    all_files_found = True
    auth = generate_auth(user, passwd)
    prefix_or_path_separator = '_' if use_version_prefix else '/'
    try:
        version_changed, remote_version = check_version(auth=auth, timeout=timeout)
        if version_changed:
            try:
                uos.mkdir('tmp')
            except:
            ...
            for filename in filenames:
                if auth:
                    response = urequests.get(f'{HOST}/{DEVICE_ID}/{remote_version}{prefix_or_path_separator}{filename}', headers={'Authorization': f'Basic {auth}'}, timeout=timeout)
                else:
                    response = urequests.get(f'{HOST}/{DEVICE_ID}/{remote_version}{prefix_or_path_separator}{filename}', timeout=timeout)
                response_status_code = response.status_code
                response_text = response.text
                response.close()
                if response_status_code != 200:
                    print(f'Remote source file {HOST}/{DEVICE_ID}/{remote_version}{prefix_or_path_separator}{filename} not found')
...
def check_for_ota_update(user=None, passwd=None, timeout=5, soft_reset_device=False):
    auth = generate_auth(user, passwd)
    version_changed, remote_version = check_version(auth=auth, timeout=timeout)
    if version_changed:
        if soft_reset_device:
